...
LIB_PATHS = ./lib/slatec

# Library names (右は左を必要としない順で並べる)
LIB_NAMES = nlopt gsl gslcblas slatec lapack blas stdc++
...
ifeq ($(FC),gfortran)
  ...
  FOPTS = -O3
  FDEBUGS = -O0 -g -fbounds-check -Wuninitialized -Wconversion  -fbacktrace -DDEBUG -ffpe-trap=zero,overflow,invalid,denormal
endif
...
ifeq ($(FC),ifort)
  ...
  FOPTS = -O3 -no-prec-div -static -fp-model fast=2 -xHost # -ipo
...
$(OBJDIR)/%.o: $(SRCDIR)/%.f90
	@-mkdir -p $(OBJDIR)
	$(FC) $(addprefix -I,$(INCLUDES)) $(FFLAGS) -o $@ -c $<
